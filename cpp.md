...
> 有名信号里
>
> [link](https://blog.csdn.net/w903414/article/details/110296964)
#### 共享内存和管道的区别 //TODO
#### 管道底层已经保证线程安全，共享内存没有保证线程安全，如果要保证线程安全该怎么做，或者说进程间同步该怎么做 //TODO
#### io多路复用的区别
> [link](https://blog.csdn.net/adminpd/article/details/124553590)
>
>  [link](https://blog.csdn.net/qq_43507406/article/details/115253064)
#### 三种内核调度策略及调度原理
> 先来先作业，短作业优先，时间片轮转调度
#### 协程的原理
> 协程主要处理IO密集型事务
>
> 协程上下文只保存相关寄存器，如栈顶指针，程序计数器等
#### 锁和信号量的应用场景/区别
> 锁没有顺序，信号量有顺序
#### python unittest
#### 结构体内存对齐问题
...
> 数组名是常量指针
>
> 当出现sizeof,和&操作符时，数组名不再当成指向一个元素的常量指针来使用。 例如数组名取地址后+1会跳过整个数组的长度 
#### 转换构造函数
> 当只有一个参数的时候，用于类型转换
#### 如何用代码判断大小端存储？
> int转char，只留下低位
...
>
> 如果使用vector，可以用vector<int>().swap()匿名对象swap来帮助释放内存，因为调用完这句匿名对象就消失了
#### 容器内部删除一个元素
...
#### RVO和NVRO
> 如果函数返回值是临时值，编译器会将对象直接构造在调用者的栈帧中(RVO 必定)
>
> 如果函数返回值是有名的局部变量，编译器会将对象直接构造在调用者的栈帧中(NRVO 需手动开启)
...
#### C++中将临时变量作为返回值时的处理过程
> 小于寄存器会使用寄存器，大于寄存器会提前在栈上申请一块内存，临时值作为返回值会触发RVO优化
